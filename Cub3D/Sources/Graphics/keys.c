...
/*   Updated: 2023/03/03 18:14:03 by yhuberla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static void	extend_key_down(int kcode, t_cub *cub)
{
	if (kcode == KEY_0 && ++cub->mlx->keys->mini_enable == 1)
...
static void	extend_key_released(int kcode, t_cub *cub)
{
	if (kcode == KEY_0)
...
int	key_down(int kcode, t_cub *cub)
{
	if (kcode == KEY_ESC)
		...
		|| kcode == KEY_W || kcode == KEY_S)
		cub->mlx->keys->vertical = (kcode == KEY_UP) + (kcode == KEY_W)
			- (kcode == KEY_DOWN) - (kcode == KEY_S);
		...
		cub->mlx->keys->fov_dist = (kcode == KEY_PLUS_PAD)
			- (kcode == KEY_MINUS_PAD);
