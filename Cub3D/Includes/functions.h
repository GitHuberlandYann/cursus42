...
void			set_fdfangles(t_angles *a, double alpha, double beta,
					double gamma);
...
void			draw_wall_vert(t_img *img, t_vert *pt, t_img *texture,
					double u);
...
void			mlx_draw_line(t_img *img, t_vert a, t_vert b,
					unsigned int color);
...
void			set_point_follow(t_vert *pt, t_vert *pt2, t_map *map,
					t_set *settings);
int				in_circle(t_vert *pt, double x, double y, double radius);
void			set_ray_angles(double fov_width, t_player *player,
					t_player *playerbis);
...
void			precompute_floor_ceilling(t_player *player,
					t_img *(fc_textures[2]));
...
void			update_anim_frames(t_map *map, t_mlx *mlx, t_key *keys,
					int fps);
