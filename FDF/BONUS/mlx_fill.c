...
/*   Updated: 2022/12/09 14:48:02 by yhuberla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	move_point(t_vertice *pt, t_vertice delta, t_vertice o)
{
	pt->x += delta.x;
	if (delta.x)
...
static void	mlx_fill_triangle(t_fdf *fdf, t_vertice a, t_vertice b, t_vertice c)
{
	...
	while (len > 0)
	{
		...
		move_point(&a, delta, starta);
		move_point(&c, deltc, startc);
		--len;
	}
}
